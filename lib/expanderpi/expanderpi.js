...
const ExpanderPiADC = (function () {

    let adc;
    let adcRefVoltage = 3.3;  //reference voltage for the ADC chip.
    let tx = new Buffer([0x01, 0xC0, 0x00]);
    let rx = new Buffer([0, 0, 0]);
    let out;
...
const ExpanderPiIO = (function () {
    ...
    ExpanderPiIO.prototype.i2cReadByte = function (val) {
        let txbuf = new Buffer([val]);
        let rxbuf = new Buffer(1);
        rpio.i2cSetSlaveAddress(this.i2caddress);
        rpio.i2cWrite(txbuf);
    ...
    let updateByte = function (oldByte, bit, value) {
    ...
    ExpanderPiIO.prototype.setPinPullup = function (pinval, value) {
        ...
        let pin = pinval - 1;
        if (pin < 8) {
            this.portAPullup = updateByte(this.portAP, pin, value);
...
module.exports = {
  ExpanderPiADC: ExpanderPiADC,
  ExpanderPiDAC: ExpanderPiDAC,
  ExpanderPiIO: ExpanderPiIO,
  ExpanderPiRTC: ExpanderPiRTC
};
